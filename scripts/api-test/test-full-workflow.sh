...
main() {
    ...
    TEST_USER_TOKEN=$(echo "$REGISTER_RESPONSE" | jq -r '.token')
    USER_ID=$(echo "$REGISTER_RESPONSE" | jq -r '.userId')
    EXPIRES_IN=$(echo "$REGISTER_RESPONSE" | jq -r '.expiresIn')

    # If the user already exists (e.g., fixed email from .env), fall back to login
    # so the workflow can still run end-to-end on repeat invocations.
    if [ -z "$TEST_USER_TOKEN" ] || [ "$TEST_USER_TOKEN" = "null" ]; then
        print_info "Registration failed; user likely exists — logging in instead..."
        REGISTER_RESPONSE=$(curl -s -X POST "${API_BASE_URL}/api/auth/login" \
            -H "Content-Type: application/json" \
            -d "{\"email\": \"$TEST_USER_EMAIL\", \"password\": \"$TEST_USER_PASSWORD\"}")
        TEST_USER_TOKEN=$(echo "$REGISTER_RESPONSE" | jq -r '.token')
        USER_ID=$(echo "$REGISTER_RESPONSE" | jq -r '.userId')
    ...
    TEST_USER_TOKEN=$(echo "$LOGIN_RESPONSE" | jq -r '.token')
    save_user_session
...
EOF
    ...
    print_info "Registering a second user: $TEST_USER2_EMAIL"
    REGISTER_RESPONSE=$(curl -s -X POST "${API_BASE_URL}/api/auth/register" \
        -H "Content-Type: application/json" \
        -d "{\"email\": \"$TEST_USER2_EMAIL\", \"password\": \"$TEST_USER2_PASSWORD\"}")

    TEST_USER2_ID=$(echo "$REGISTER_RESPONSE" | jq -r '.userId')

    if [ -z "$TEST_USER2_ID" ] || [ "$TEST_USER2_ID" = "null" ]; then
        print_info "Second-user registration failed; logging in instead..."
        REGISTER_RESPONSE=$(curl -s -X POST "${API_BASE_URL}/api/auth/login" \
            -H "Content-Type: application/json" \
            -d "{\"email\": \"$TEST_USER2_EMAIL\", \"password\": \"$TEST_USER2_PASSWORD\"}")
        TEST_USER2_ID=$(echo "$REGISTER_RESPONSE" | jq -r '.userId')
    ...
    print_success "Second user ready: $TEST_USER2_ID"
    ...
    SHARE_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
        ...
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer $TEST_USER_TOKEN" \
        -d "{\"userId\": \"$TEST_USER2_ID\", \"role\": \"viewer\"}")
    ...
    echo -e "  Shared Savings with:    $TEST_USER2_ID (viewer)"
    echo ""
