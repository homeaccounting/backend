...
--
--  1. Within-kind Income amount rescale (newAllocations = Nothing).
...
spec = describe "amendTransaction — cross-kind synthesis" $ do
  it "(1) within-kind amount-only Income rescale" $ do
    ...
    -- Same accounts and kind; amount bumped to 150; no new allocations.
    let cmd =
          buildAmend
            txId
    ...
    case result of
      Right td -> do
        ...
        -- Income kind preserved; single allocation rescaled 100 → 150.
        td.transactionType
  ...
  it "(6) rejects allocation with categoryId not in user's income dictionary" $ do
    ...
    let unknownEntryId =
        ...
        badAllocs = Allocation unknownEntryId (unsafeMoney Core.USD 100) :| []
        cmd =
          buildAmend
            txId
            original.sourceAccountId
