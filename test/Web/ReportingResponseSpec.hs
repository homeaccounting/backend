...
spec = describe "Reporting DTOs" $ do
  it "CategorySpend renders amount as a nested Money object" $ do
    let cs = CategorySpend {categoryId = "cat-1", total = toMoneyDTO (unsafeMoney UAH 1234)}
  ...
  it "IncomeVsExpenseResponse carries income/expense/net as Money" $ do
    let r = IncomeVsExpenseResponse {income = toMoneyDTO (unsafeMoney UAH 500), expense = toMoneyDTO (unsafeMoney UAH 200), net = toMoneyDTO (unsafeMoney UAH 300)}
  ...
  it "SpendingByCategoryResponse nests categories and a base total" $ do
    let r = SpendingByCategoryResponse {categories = [], total = toMoneyDTO (unsafeMoney UAH 0)}
