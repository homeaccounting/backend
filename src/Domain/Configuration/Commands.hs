...
--
-- The token is **already encrypted** by the time this command is issued —
...
data AddBankConnection = AddBankConnection
  { -- | Freshly generated identifier for the new connection
    ...
    -- | The already-encrypted provider token
...
-- | Command to change an existing bank connection's token.
--
-- The token is **already encrypted** (re-encrypted in the service layer).
-- If accepted, produces a
-- 'Domain.Configuration.Events.BankConnectionTokenChanged' event.
data ChangeBankConnectionToken = ChangeBankConnectionToken
  { connectionId :: BankConnectionId,
    encryptedToken :: EncryptedSecret,
    tokenHint :: Text
  }
...
deriveJSON defaultOptions ''ChangeBankConnectionToken
