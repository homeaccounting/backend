  ...
  let rawWriter = postgresqlTaggedEventStoreWriter config
      ...
      combinedHandler =
        eventLoggerHandler
          ...
          -- Process manager receives rawWriter (not publishingWriter) intentionally:
          -- commands it dispatches must not re-enter the publishing event bus.
          <> transferManagerHandler rawWriter globalReader versionedReader
