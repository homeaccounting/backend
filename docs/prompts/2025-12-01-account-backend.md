...
### Infrastructure
* Use `rio` library for application monad, dependency handling and resource management

### Dependencies

* `mtl` - MonadIO, monad transformers
* `monad-logger` - structured logging
* `persistent` - SqlPersistT for database
* `eventium` - event sourcing library
* `servant` & `warp` - for web layer
* `rio` - application monad
