...
Imports: `T` = `RIO.Text`; `isDigit` from `RIO.Char`; `listToMaybe` from `RIO.List`; `NominalDiffTime`/`diffUTCTime` from `Data.Time`; `unExternalAccountId` from `Domain.Banking.Types`; `TransferMatcher (..)`, `TransactionInterpretation (..)`, `defaultClassify`, `defaultTransferPairingWindow` from `Infrastructure.Banking.Provider`. (`last4` filters to digits first so the masked format `4627 **** **** 9713` yields `9713`.)
...
- `ownCardCounterpartLast4` returns `Just "9713"` for description `На свою картку *9713`, `Just "1440"` for `Зі своєї картки *1440`, and `Nothing` for a normal row (e.g. `Сидоренко Р.`) and for a ФОП `Зарахування … Переказ власних коштiv` row (no card).
- `matchesTransfer (privatBankTransferMatcher defaultTransferPairingWindow)` is `True` for the outgoing `…1440 / *9713 / −20000 UAH` leg against the incoming `…9713 / *1440 / +20000 UAH` leg (1 s apart), and `False` when the named last-4 doesn't match the other card, when magnitudes differ, when currencies differ, or when one leg is a non-self-labeled row.
...
2. **File / PrivatBank merged multi-account batch.** Frame as the file transport with the PrivatBank interpretation: `importMany privatBankInterpretation userId accountLink batch` (this is what `importStatementFileHandler` does with a merged statement). Model the real data as synthetic `BankTransaction`s: card `…1440` self-labeled `На свою картку *9713 −20000` and card `…9713` self-labeled `Зі своєї картки *1440 +20000` (1 s apart), with `accountLink` mapping `…1440 → A`, `…9713 → B`, and a **sibling card `…9959 → B`** (same local account as `…9713`). Expect: (a) exactly one `Transfer` A→B for the labeled pair; (b) a within-account `…9713`↔`…9959` equal-amount opposite-sign decoy pair is **not** collapsed (different cards, same local account B). Proves provider/transport-neutrality and the many-cards-per-account invariant without a real CSV.
